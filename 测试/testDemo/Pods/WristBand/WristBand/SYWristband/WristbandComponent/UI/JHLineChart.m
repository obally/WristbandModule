...
//            [weakself addGradientLayer:secondPath.CGPath color:weakself.contentFillColorArr[colorIndex]];
//        }else{
//            [weakself addGradientLayer:secondPath.CGPath color:[UIColor clearColor]];
//        }
//        
//        
//        
...
- (void)addGradientLayer:(CGPathRef)path color:(UIColor *)color
{
    CAShapeLayer *shadeLayer = [CAShapeLayer layer];
    shadeLayer.path = path;
    shadeLayer.fillColor = [UIColor greenColor].CGColor;
    
    CAGradientLayer *gradientLayer = [CAGradientLayer layer];
    gradientLayer.frame = CGRectMake(5, 0, self.bounds.size.width, self.bounds.size.height-20);
...
-(void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag{
    ...
    if (flag) {
        ...
        shaperLay.path = self.secondPath.CGPath;
        if (self.contentFillColorArr.count == self.drawDataArr.count) {
            [self addGradientLayer:self.secondPath.CGPath color:self.contentFillColorArr[0]];
        }else{
            [self addGradientLayer:self.secondPath.CGPath color:[UIColor clearColor]];
        }
...
- (void)drawPoint{
    ...
    for (NSInteger m = 0;m<_drawDataArr.count;m++) {
        ...
        for (NSInteger i = 0; i<arr.count; i++) {
            ...
            UIBezierPath *pBezier1 = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(0, 0, 10, 10)];
            CAShapeLayer *pLayer1 = [CAShapeLayer layer];
            pLayer1.frame = CGRectMake(0, 0, 10, 10);
            pLayer1.position = p;
            pLayer1.path = pBezier1.CGPath;
            
            UIColor *color1 = [UIColor colorWithRed:61.0/255 green:201.0/255 blue:255.0/255 alpha:0.268];
            
            pLayer1.fillColor = color1.CGColor;
            
            CABasicAnimation *ani1 = [CABasicAnimation animationWithKeyPath:NSStringFromSelector(@selector(strokeEnd))];
            ...
            [pLayer1 addAnimation:ani1 forKey:NSStringFromSelector(@selector(strokeEnd))];
            [self.layer addSublayer:pLayer1];
            [_layerArr addObject:pLayer1];
